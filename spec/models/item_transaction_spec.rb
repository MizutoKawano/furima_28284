...
it "token, arium_id, city, house_number, tellnum, postnumがあれば購入できる"do
...
@item_transaction.valid?
expect(@item_transaction.errors.full_messages).to include("House number can't be blank")
end
it "電話番号が空と保存できない"do  
@item_transaction.tellnum = ""
@item_transaction.valid?
expect(@item_transaction.errors.full_messages).to include("Tellnum can't be blank")
end
...
@item_transaction.valid?
expect(@item_transaction.errors.full_messages).to include()
end
